...
>> %LOG_FILE_PATH% (
	...
	robocopy /np /njh /njs "Assets/%ROOT_FOLDER_NAME%/Editor/Scripts/Utils" "%OUTPUT_PATH%/Editor/Scripts/Utils" /xf "*.meta"
	robocopy /np /njh /njs "Assets/%ROOT_FOLDER_NAME%/Editor/Scripts" "%OUTPUT_PATH%/Editor/Scripts" "HTC.UPMRegistryTool.Editor.asmdef"
	robocopy /np /njh /njs /s "Assets/%ROOT_FOLDER_NAME%/Plugins" "%OUTPUT_PATH%/Plugins" /xf "*.meta"
